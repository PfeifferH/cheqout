...
def get_prices(inventory, item):
    ...
    for document in inventory.get():
        if document.id == item:
            ...
            return item_data['unit_price'], item_data['tax']
    return None

def store_transaction(carts, transactions, inventory, cart_id):
    """
    ...
    transaction_data = {"cart": cart_id, "items": [], "subtotal": 0., "tax": 0., "total": 0.}
    for item in item_array:
        # TODO: handle case where item prace wasn't found
        if get_prices(inventory, item['id']) is None:
            print("PRICE NOT FOUND")
        else:
            unit_price, tax = get_prices(inventory, item['id'])
            transaction_data['subtotal'] += float(unit_price) * item['quantity']
            transaction_data['tax'] += float(tax) * item['quantity']
            transaction_data['items'].append({'id' : item['id'], 'quantity' : item['quantity']})
    ...
    transaction_data['payment_type'] = 'TEST PAYMENT TYPE'
